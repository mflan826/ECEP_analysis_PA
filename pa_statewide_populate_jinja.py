...
pa_statewide_output_file_name = f"{file_path_prefix}/{config['pa_statewide_output_file_name']}"
...
with open('pa_statewide_jinja.sql') as file:
    template = Template(file.read())

for dash, splat in zip(school_year_dash, school_year_splat):
    rendered = template.render(school_year_dash=dash, school_year_splat=splat)
    cursor = conn.cursor()
...
write_dataframes(pa_statewide_output_file_name, "Tab 2 -Statewide Data", agg_dfs)
